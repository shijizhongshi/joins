	...
	public Results<String> addupdateUserFinish(UserFinishDomain userFinishDomain) {
		...
		try {
			 if(userFinishDomain.getExamBeanList().get(0).getTypes()==1){
				 String courseTypeSubclassName=questionSubcategoryDao.showSubName(userFinishDomain.getExamBeanList().get(0).getSubId());
				 UserFinishDays existDays=userFinishDaysDao.UserFinishDaysSingle(userFinishDomain.getUserId(), courseTypeSubclassName);
				 if(existDays==null){
					 int userfinsih=0;
					...
					userFinishDays.setCourseTypeSubclassName(courseTypeSubclassName);
					 ...
					 SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
						Date time=questionBankUserFinishDao.maxAddtime(userFinishDomain.getUserId(), userFinishDomain.getExamBeanList().get(0).getSubId());
