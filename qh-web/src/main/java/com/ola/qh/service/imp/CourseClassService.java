...
public class CourseClassService implements ICourseClassService {
	 ...
	 */
	@Override
	public Results<CourseClassVo> classSingleAll(String classId, String userId, String address) {
		//复制上面的接口  在for循环中添加for循环
		...
		for (Course course : clist) {
			//根据课程id查询章
			List<CourseChapter> chapterList = courseDao.courseChapterList(course.getId());
			for (CourseChapter courseChapter : chapterList) {
				//根据章id查询节
				List<CourseSection> serviceList = courseDao.courseSectionList(courseChapter.getId());
				courseChapter.setCourseSections(serviceList);
			}
			course.setCourseChapters(chapterList);
